...
import { 
  FiGrid, FiShoppingBag, FiUsers, FiBox, FiSettings, 
  FiMenu, FiX, FiLogOut, FiBarChart2, FiUser, FiBell 
} from "react-icons/fi";
...
const DashboardLayout = () => {
  const { user: firebaseUser, logOut, loading: authLoading } = useContext(AuthContext);
  ...
  if (authLoading || dbLoading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-white">
        ...
        <p className="mt-4 font-bold text-gray-500 animate-pulse">Loading Workspace...</p>
      </div>
    );
  }
  ...
  const adminMenu = [
    ...
    { name: "Users", path: "/dashboard/admin/manage-user", icon: <FiUsers size={20} /> },
    { name: "Manage Products", path: "/dashboard/admin/manage-products", icon: <FiBox size={20} /> },
    { name: "Add Products", path: "/dashboard/admin-manger/add-products", icon: <FiBox size={20} /> },
    { name: "Manage Orders", path: "/dashboard/manage-orders", icon: <FiBox size={20} /> },
  ];

  const managerMenu = [
    { name: "Inventory", path: "/dashboard/inventory", icon: <FiBox size={20} /> },
    { name: "Orders", path: "/dashboard/orders", icon: <FiShoppingBag size={20} /> },
  ];

  const userMenu = [
    { name: "My Orders", path: "/dashboard/my-orders", icon: <FiShoppingBag size={20} /> },
  ...
  else if (dbUser?.role === "manager") finalMenu = [...managerMenu, ...commonMenu];
  else finalMenu = [...userMenu, ...commonMenu];

  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white">
      <div className="p-8">
        <Link to="/" className="text-2xl font-black text-green-600 tracking-tighter flex items-center gap-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <h2 className="text-sm font-black text-gray-800 uppercase tracking-widest opacity-40">
                Dashboard / <span className="text-green-600 opacity-100">{dbUser?.role}</span>
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden md:block">
                <p className="text-[13px] font-black text-gray-900 leading-none">{dbUser?.fullName}</p>
              ...
              <div className="relative group cursor-pointer">
                <img 
                  src={dbUser?.photoURL || `https://ui-avatars.com/api/?name=${dbUser?.fullName}&background=22c55e&color=fff`} 
