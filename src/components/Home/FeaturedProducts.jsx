...
const FeaturedProducts = () => {
  ...
  const fetchProducts = async (category = "") => {
    try {
      ...
      const url = `http://localhost:3000/products?limit=8${category ? `&category=${category}` : ""}`;
  ...
  return (
    <section className="py-10 md:py-16 bg-gray-50/50">
      <div className="container mx-auto px-2 md:px-4">
            ...
            {["", "Vegetables", "Fruits", "Dairy"].map((cat) => (
              <button
                ...
                className={`pb-2 px-2 font-bold text-sm md:text-base transition-colors ${
                  filter === cat ? "border-b-2 border-green-600 text-green-600" : "text-gray-400 hover:text-gray-800"
                }`}
              >
        ...
        {loading ? (
          <div className="text-center py-20"><span className="loading loading-ring loading-xl"></span></div>
        ) : (
