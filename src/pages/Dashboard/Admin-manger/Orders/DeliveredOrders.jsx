...
const DeliveredOrders = () => {
  ...
  const fetchDeliveredOrders = async () => {
    ...
    try {
      ...
      let deliveredOrders = res.data.orders.filter(
        ...
        <div className="bg-green-600 p-6 md:p-8 text-white">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              ...
              <p className="text-green-100 text-xs uppercase font-bold tracking-widest mt-1">
                Successfully Completed • {orders.length} orders
              </p>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3 lg:hidden">
                {orders.map((order) => (
                  <div
                    key={order._id}
                    className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col justify-between gap-3 relative overflow-hidden"
                  >
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <span className="bg-green-50 text-green-600 px-2 py-0.5 rounded text-[8px] font-black uppercase border border-green-100">
                          Paid
                        </span>
                    ...
                    <Link
                      to={`/dashboard/admin/order-details/${order._id}`}
                      className="w-full py-2 bg-gray-900 text-white rounded-lg flex justify-center items-center gap-2 text-[10px] font-bold cursor-pointer hover:bg-green-600 transition-colors"
                    >
