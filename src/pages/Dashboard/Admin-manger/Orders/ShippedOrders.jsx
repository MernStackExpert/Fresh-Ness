...
const ShippedOrders = () => {
  ...
  const fetchShippedOrders = async () => {
    ...
    try {
      ...
      let shippedOrders = res.data.orders.filter((order) => order.orderStatus === "shipped");
  ...
  const markAsDelivered = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axiosInstance.patch(`/orders/${id}`, { orderStatus: "delivered" });
        if (res.status === 200) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-2 md:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="bg-blue-600 p-6 md:p-8 text-white">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
              ...
              <p className="text-blue-100 text-xs uppercase font-bold tracking-widest mt-1">In Transit • {orders.length} orders</p>
            </div>
        ...
        <div className="p-4 md:p-6">
          {loading ? (
            <div className="py-20 text-center font-black animate-pulse text-gray-300 uppercase tracking-widest">Tracking Packages...</div>
          ) : orders.length === 0 ? (
            <div className="py-20 text-center text-gray-400 font-bold uppercase tracking-widest">No Shipped Orders in Transit</div>
          ) : (
            <>
                      ...
                      {orders.map((order) => (
                        ...
                        >
                          <td className="px-6 py-4 rounded-l-2xl border-y border-l border-gray-100">
                            ...
                            <div className="text-[9px] text-gray-400 font-mono">ORDER-ID: {order._id.slice(-10)}</div>
                          </td>
                          <td className="px-6 py-4 border-y border-gray-100">
                            <div className="text-sm font-black text-blue-600">${order.total}</div>
                            <div className="text-[10px] text-gray-400 font-bold uppercase">Ready to Receive</div>
                          </td>
                          <td className="px-6 py-4 border-y border-gray-100 text-center">
                            <div className={`px-4 py-1 rounded-full text-[9px] font-black uppercase border ${order.paymentStatus === "paid" ? "bg-green-50 text-green-600 border-green-100" : "bg-amber-50 text-amber-600 border-amber-100"}`}>
                              {order.paymentStatus}
                            </div>
                          </td>
                  ...
                  {orders.map((order) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-[10px] font-bold text-gray-800 truncate">{order.email}</h4>
                          <p className="text-[9px] text-blue-600 font-black">${order.total}</p>
                        </div>
                        <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[8px] font-black uppercase">Shipped</span>
                      </div>
                      <div className="flex gap-2 pt-2 border-t border-gray-50">
                        <Link to={`/dashboard/admin/order-details/${order._id}`} className="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg flex justify-center items-center"><FaEye size={12} /></Link>
                        <button onClick={() => markAsDelivered(order._id)} className="flex-2 py-2 bg-green-600 text-white rounded-lg flex justify-center items-center gap-2 text-[10px] font-bold">
